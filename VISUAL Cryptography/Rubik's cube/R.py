...
def encrypt_image():
 ...
 for iterations in range(ITER_MAX):
  ...
  for i in range(n):
   ...
   for j in range(m):
    ...
    bModulus = bTotalSum % 2
    if(rModulus==0):
     helper.upshift(r,i,Kc[i])
    else:
     helper.downshift(r,i,Kc[i])
    if(gModulus==0):
     helper.upshift(g,i,Kc[i])
    else:
     helper.downshift(g,i,Kc[i])
    if(bModulus==0):
     helper.upshift(b,i,Kc[i])
    else:
     helper.downshift(b,i,Kc[i])
	 # For each row
  for i in range(m):
   for j in range(n):
    if(i%2==1):
 ...
 print('saved in encrypted_images')
